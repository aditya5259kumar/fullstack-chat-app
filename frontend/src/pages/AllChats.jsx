...
//   const convoExists = inboxData.some(
//     (c) => String(c.conversation_id) === String(newMessage.conversation_id),
//   );

//   if (!convoExists) {
//     // Brand new conversation B has never seen before — only way to get
//     // its participant/user info is a fetch. This only happens once per
//     // new conversation, not on every message.
//     dispatch(userConversation());
//     return;
//   }
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (newMessage) => {
      const convoExists = inboxDataRef.current.some(
        (c) =>
          String(c.conversation_id) === String(newMessage.conversation_id),
      );
